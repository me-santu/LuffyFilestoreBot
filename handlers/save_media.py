...
async def save_batch_media_in_channel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = await get_shortlink(f"https://t.me/{Config.BOT_USERNAME}?start=MxABots_{str_to_b64(str(SaveMessage.id))}")
        await editable.edit(
...
async def save_media_in_channel(bot: Client, editable: Message, message: Message):
    try:
        ...
        await forwarded_msg.reply_text(
            ...
            disable_web_page_preview=True)
        share_link = await get_shortlink(f"https://t.me/{Config.BOT_USERNAME}?start=MxABots_{str_to_b64(file_er_id)}")
        await editable.edit(
